...
$return_tab = isset($_GET['tab']) ? $_GET['tab'] : 'affiliated_cars';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['id']) && !empty($_POST['id'])) {
        ...
        if (!empty($logo_path)) {
            $sql = "UPDATE affiliated_cars SET 
                    name = '$name',
        ...
        // Insert new record
        if (empty($logo_path)) {
            $_SESSION['error'] = "Please upload a logo for the car company!";
            header("Location: admin_dashboard.php?tab=$return_tab");
            exit();
        }
        
        $sql = "INSERT INTO affiliated_cars (name, display_order, is_active, logo_path) 
