...
$return_tab = isset($_GET['tab']) ? $_GET['tab'] : 'affiliated_banks'; // Get the tab to return to
...
// Fetch the bank data
$sql = "SELECT * FROM affiliated_banks WHERE id = $id";
...
<head>
    ...
    <title>Edit Bank - Hope Account Specialist</title>
...
<body>
    <div class="edit-container">
        ...
        <div class="current-logo-section">
            <div class="current-logo-label">
                <i class="fas fa-image"></i> Current Bank Logo
            </div>
        ...
        <!-- Edit Form -->
        <form method="POST" action="admin_save_bank.php?tab=<?php echo $return_tab; ?>" enctype="multipart/form-data" class="edit-form">
            <input type="hidden" name="id" value="<?php echo $bank['id']; ?>">
            <input type="hidden" name="return_tab" value="<?php echo $return_tab; ?>">
            
            <div class="form-group full-width">
                <label for="name">
                    <i class="fas fa-university"></i> Bank Name <span style="color: var(--gold-primary);">*</span>
                </label>
                <input type="text" id="name" name="name" class="form-control" 
                       value="<?php echo htmlspecialchars($bank['name']); ?>" 
                       placeholder="e.g., BDO Unibank, Metrobank, BPI" required>
                <div class="helper-text">
                    <i class="fas fa-info-circle"></i> Full official name of the bank
                </div>
            </div>
            ...
            <div class="form-group full-width">
                ...
                <input type="number" id="display_order" name="display_order" class="form-control"
                       value="<?php echo $bank['display_order']; ?>" min="0" step="1">
                <div class="helper-text">
            ...
            <div class="form-group full-width">
                ...
                <div class="status-toggle">
                    <label class="status-option <?php echo $bank['is_active'] ? 'active-option' : ''; ?>">
                        <input type="radio" name="is_active" value="1" <?php echo $bank['is_active'] ? 'checked' : ''; ?>>
                        <span class="status-badge-demo active">
                            <i class="fas fa-check-circle"></i> Active
                        </span>
                        <span style="color: var(--text-muted); font-size: 0.85rem;">Display on website</span>
                    </label>
                    <label class="status-option <?php echo !$bank['is_active'] ? 'inactive-option' : ''; ?>">
                        <input type="radio" name="is_active" value="0" <?php echo !$bank['is_active'] ? 'checked' : ''; ?>>
            ...
            <!-- Bank Info Card (Helpful Tip) -->
            <div class="info-card">
                <p>
                    <i class="fas fa-lightbulb"></i> 
                    <strong style="color: var(--gold-primary);">Bank Partner Tip:</strong> 
                    Ensure the bank logo is clear and professional. Active banks will appear in the "Affiliated Banks" section on the main website.
                </p>
            </div>
            ...
            <div class="form-actions">
                ...
                <a href="?tab=<?php echo $return_tab; ?>&delete=<?php echo $bank['id']; ?>" 
                   class="btn btn-danger" 
                   onclick="return confirmDelete()">
                    <i class="fas fa-trash-alt"></i> Delete Bank
                </a>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        function confirmDelete() {
            return confirm('⚠️ Are you sure you want to delete this bank?\n\nBank: <?php echo addslashes($bank['name']); ?>\n\nThis action cannot be undone and will permanently remove the bank from your affiliated banks list.');
        }
        ...
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                if (!nameInput.value.trim()) {
                    e.preventDefault();
                    showNotification('error', 'Please enter the bank name');
                    nameInput.focus();
                    return false;
                }
